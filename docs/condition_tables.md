...
To use a condition table, set `condition_table_path` in the `advanced` options of an [`ND2PlateAcquisitionModel`](converters/nd2_plate.md):

```python
from fractal_nd2_converters import ND2PlateAcquisitionModel
from ome_zarr_converters_tools import AcquisitionOptions

ND2PlateAcquisitionModel(
    path="/path/to/plate_folder",
    advanced=AcquisitionOptions(condition_table_path="/path/to/condition_table.csv"),
)
```
