...
Fractal tasks to convert Nikon ND2 files into OME-Zarr. Conversion engine lives in
...
- `pixi run -e dev fractal-manifest check --package fractal_nd2_converters` — validate the Fractal manifest

## Testing

Snapshot-based via the shared `ome_zarr_converters_tools.testing` helper; reference
JSON in `tests/data/Nikon-ND2/snapshots/`. Regenerate with `--update-snapshots`. The
...
`tests/data-extended/`; `tests/conftest.py` also extracts the local ND2 zips there
before collection.
